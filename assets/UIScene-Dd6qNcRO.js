import{P as h}from"./phaser-CeIIdTfV.js";import{W as l,g as p,d as f,e as u,C as v,B as M,I as r,f as S,i as w}from"./index-BQmObesC.js";class T extends h.Scene{root;profile;progress;helpOpen=!1;debugOpen=!1;shopOpen=!1;connection={status:"connecting",message:"Connecting to WULAND server",totalPlayers:0,onlinePlayers:0,sleepingPlayers:0,totalEnemies:0,aliveEnemies:0,localHp:0,localMaxHp:0,localShield:0,defeated:!1,inventory:Array.from({length:f},(t,e)=>({slotIndex:e,itemDefinitionId:"",itemInstanceId:"",quantity:0})),selectedHotbarSlot:0,money:0,activeItemName:"No item",nearbyPickupName:"",nearMerchant:!1,nearbyPortalId:"",portalPrompt:"",nearbyGiftPlayerName:"",currentMapId:l,currentMapName:p(l),totalDroppedItems:0,godModeAvailable:!1,godModeCodeRequired:!1,godModeActive:!1,serverProtocolVersion:0,serverProtocolOk:!1};hotbarDrag;chatMessages=[];chatCollapsed=!1;godModeCode="";shopFeedback="";lastBuyIntent=null;constructor(){super("UIScene")}create(t){this.profile=t.profile,this.progress=t.progress,this.connection=t.connection??this.connection,this.chatCollapsed=o()&&!k(),this.mount(),this.render(),this.game.events.on("wuland:progressUpdated",this.handleProgressUpdated,this),this.game.events.on("wuland:connectionUpdated",this.handleConnectionUpdated,this),this.game.events.on("wuland:toggleHelp",this.toggleHelp,this),this.game.events.on("wuland:toggleDebug",this.toggleDebug,this),this.game.events.on("wuland:openMerchantShop",this.openMerchantShop,this),this.game.events.on("wuland:chatHistory",this.handleChatHistory,this),this.game.events.on("wuland:chatMessage",this.handleChatMessage,this),this.game.events.on("wuland:shopFeedback",this.handleShopFeedback,this),this.game.events.on("wuland:focusChat",this.focusChatInput,this),this.events.once(h.Scenes.Events.SHUTDOWN,this.cleanup,this)}mount(){const t=document.getElementById("ui-root");if(!t)throw new Error("WULAND UI root is missing.");this.root=document.createElement("div"),this.root.className="wuland-hud",this.root.innerHTML=`
      ...
      <section class="chat-window" data-chat-window>
        ...
        <form class="chat-form" data-chat-form>
          <input data-chat-input maxlength="${u}" autocomplete="off" placeholder="Enter to chat" />
    ...
    `,t.appendChild(this.root),this.root.querySelector('[data-action="edit-character"]')?.addEventListener("click",()=>{this.game.events.emit("wuland:editCharacter")}),this.root.querySelector('[data-action="help"]')?.addEventListener("click",()=>this.toggleHelp()),this.root.querySelector('[data-action="god-mode"]')?.addEventListener("click",()=>this.toggleGodMode()),this.root.querySelector('[data-action="close-help"]')?.addEventListener("click",()=>this.toggleHelp(!1)),this.root.querySelector('[data-action="toggle-chat"]')?.addEventListener("click",()=>this.toggleChat()),this.root.querySelector("[data-chat-form]")?.addEventListener("submit",e=>this.handleChatSubmit(e)),this.root.querySelector("[data-chat-input]")?.addEventListener("keydown",e=>this.handleChatInputKeydown(e)),this.root.querySelector('[data-action="close-shop"]')?.addEventListener("click",()=>this.openMerchantShop(!1)),this.root.querySelector("[data-merchant-stock]")?.addEventListener("pointerup",e=>this.handleShopBuyIntent(e)),this.root.querySelector("[data-merchant-stock]")?.addEventListener("click",e=>this.handleShopBuyIntent(e)),this.root.querySelector("[data-hotbar-slots]")?.addEventListener("pointerdown",e=>this.handleHotbarPointerDown(e)),window.addEventListener("pointermove",this.handleHotbarPointerMove),window.addEventListener("pointerup",this.handleHotbarPointerUp),window.addEventListener("keydown",this.handleWindowKeydown,!0)}render(){if(!this.root)return;const t=v[this.profile.class],e=new Set(this.progress.visitedBuildings),a=this.root.querySelector("[data-hud-buildings]");this.setText("[data-hud-name]",this.profile.name),this.setText("[data-hud-class]",`${t.iconText} ${t.displayName} | ${t.futureRole}`),this.setText("[data-hud-location]",this.connection.currentMapName),this.setText("[data-hud-position]",`x:${Math.round(this.progress.lastPosition.x)} y:${Math.round(this.progress.lastPosition.y)}`),this.setText("[data-hud-save]",`saved ${new Date(this.progress.updatedAt).toLocaleTimeString()}`),this.setText("[data-hud-connection]",this.connection.message),this.setText("[data-hud-total]",String(this.connection.totalPlayers)),this.setText("[data-hud-online]",String(this.connection.onlinePlayers)),this.setText("[data-hud-sleeping]",String(this.connection.sleepingPlayers)),this.setText("[data-hud-hp]",`${this.connection.localHp}/${this.connection.localMaxHp}${this.connection.defeated?" respawning":""}`),this.setText("[data-hud-active-item]",this.connection.activeItemName),this.setText("[data-hud-money]",`${g(this.connection.money)} coins`),this.setText("[data-shop-money]",`${g(this.connection.money)} WULAND coins`),this.setText("[data-shop-feedback]",this.shopFeedback),this.setText("[data-hud-pickup-hint]",this.interactionHint()),this.setText("[data-hud-enemies]",String(this.connection.totalEnemies)),this.setText("[data-hud-alive-enemies]",String(this.connection.aliveEnemies)),this.setText("[data-hud-shield]",String(this.connection.localShield)),this.setMeter("[data-hud-hp-fill]",this.hpPercent()),this.root.dataset.connectionStatus=this.connection.status,this.root.dataset.helpOpen=String(this.helpOpen),this.root.dataset.debugOpen=String(this.debugOpen),this.root.dataset.shopOpen=String(this.shopOpen),this.root.dataset.chatCollapsed=String(this.chatCollapsed),this.root.dataset.godModeActive=String(this.connection.godModeActive),this.root.dataset.touchLayout=String(o()),this.setGodModeButton(),this.setChatButton(),this.renderHotbar(),this.renderMerchantStock(),this.renderChatMessages(),a&&(a.innerHTML=M.map(n=>{const d=e.has(n)?"[x]":"[ ]";return`<li class="${e.has(n)?"visited":""}"><span>${d}</span>${n}</li>`}).join(""))}handleProgressUpdated(t){this.progress=t,this.render()}handleConnectionUpdated(t){this.connection=t,this.render()}toggleHelp(t){this.helpOpen=t??!this.helpOpen,this.render()}toggleDebug(){this.debugOpen=!this.debugOpen,this.render()}openMerchantShop(t=!0){this.shopOpen=t,t&&(this.shopFeedback=""),this.render()}toggleChat(t){this.chatCollapsed=t??!this.chatCollapsed,this.chatCollapsed?this.root?.querySelector("[data-chat-input]")?.blur():o()&&window.setTimeout(()=>this.focusChatInput(),0),this.render()}focusChatInput(){this.chatCollapsed=!1,this.render(),this.root?.querySelector("[data-chat-input]")?.focus()}toggleGodMode(){if(!this.connection.godModeAvailable)return;const t=!this.connection.godModeActive;if(t&&this.connection.godModeCodeRequired&&!this.godModeCode){const e=window.prompt("Enter God Mode code");if(!e)return;this.godModeCode=e}this.game.events.emit("wuland:setGodMode",{active:t,code:this.godModeCode})}setGodModeButton(){const t=this.root?.querySelector('[data-action="god-mode"]');t&&(t.disabled=!this.connection.godModeAvailable,t.textContent=this.connection.godModeActive?"God: On":"God Mode",t.title=this.connection.godModeActive?"God Mode: click a dropped item to delete it, or another player to delete their account.":"Prototype admin cleanup tool.")}setChatButton(){const t=this.root?.querySelector('[data-action="toggle-chat"]');t&&(t.textContent=this.chatCollapsed?"Open":"Min")}handleChatMessage(t){this.chatMessages=m(this.chatMessages,[t]),this.render()}handleChatHistory(t){this.chatMessages=m(this.chatMessages,t),this.render()}handleShopFeedback(t){this.shopFeedback=t,this.render()}handleChatSubmit(t){t.preventDefault();const e=this.root?.querySelector("[data-chat-input]"),a=e?.value.trim()??"";if(e){if(a.length===0){e.blur();return}this.game.events.emit("wuland:sendChat",{text:a.slice(0,u)}),e.value="",e.blur(),document.activeElement instanceof HTMLElement&&document.activeElement.blur(),this.render()}}handleChatInputKeydown(t){t.stopPropagation(),t.key==="Escape"&&(t.preventDefault(),this.root?.querySelector("[data-chat-input]")?.blur())}handleWindowKeydown=t=>{if(t.key!=="Enter"&&t.key!=="Escape")return;const e=this.root?.querySelector("[data-chat-input]");if(e){if(t.key==="Escape"&&document.activeElement===e){t.preventDefault(),t.stopPropagation(),e.blur();return}t.key==="Enter"&&document.activeElement!==e&&(t.preventDefault(),t.stopPropagation(),this.chatCollapsed=!1,this.render(),e.focus())}};interactionHint(){const t=[];return this.connection.nearMerchant?t.push(o()?"Interact: shop":"F: shop"):this.connection.portalPrompt?t.push(o()?C(this.connection.portalPrompt):this.connection.portalPrompt):this.connection.nearbyPickupName&&t.push(o()?`Interact: pick up ${this.connection.nearbyPickupName}`:`F: pick up ${this.connection.nearbyPickupName}`),this.connection.nearbyGiftPlayerName&&t.push(o()?`Gift: ${this.connection.nearbyGiftPlayerName}`:`G: gift to ${this.connection.nearbyGiftPlayerName}`),t.join(" | ")}setText(t,e){const a=this.root?.querySelector(t);a&&(a.textContent=e)}setMeter(t,e){const a=this.root?.querySelector(t);a&&(a.style.width=`${Math.round(e*100)}%`)}hpPercent(){return this.connection.localMaxHp<=0?0:Math.max(0,Math.min(1,this.connection.localHp/this.connection.localMaxHp))}renderHotbar(){const t=this.root?.querySelector("[data-hotbar-slots]");t&&(t.innerHTML=this.connection.inventory.map(e=>{const a=e.itemDefinitionId?r[e.itemDefinitionId]:null,n=e.slotIndex===this.connection.selectedHotbarSlot,d=a?.stackable&&e.quantity>1?`<span class="hotbar-count">${e.quantity}</span>`:"",i=a?`${a.displayName} (${a.itemType}): ${a.description} ${P(a.itemDefinitionId)}`:`Empty slot ${e.slotIndex+1}`;return`
      ...
      `}).join(""))}renderMerchantStock(){const t=this.root?.querySelector("[data-merchant-stock]");t&&(t.innerHTML=S.map(e=>{const a=r[e.itemDefinitionId],n=I(this.connection.inventory,e.itemDefinitionId),d=this.connection.money>=e.price,i=n?d?"":"Need more coins":"Inventory full";return`
        <article class="merchant-item${i?" blocked":""}">
          <span class="merchant-icon">${y(a)}</span>
          <div>
            <h3>${a.displayName}</h3>
          ...
          <button
            type="button"
            class="primary small"
            data-buy-item="${a.itemDefinitionId}"
            ${i?"disabled":""}
            title="${i||`Buy ${a.displayName}`}"
          >${i||"Buy"}</button>
        </article>
      `}).join(""))}renderChatMessages(){const t=this.root?.querySelector("[data-chat-messages]");t&&(t.innerHTML=this.chatMessages.map(e=>`
        <p>
          ${e.mapId===this.connection.currentMapId?"":`<span class="chat-map">[${c(p(e.mapId))}]</span>`}
          <strong>${c(e.playerName)}</strong>
          <span>${c(e.text)}</span>
        </p>
      `).join(""),t.scrollTop=t.scrollHeight)}handleShopBuyIntent(t){const e=t.target?.closest("[data-buy-item]"),a=e?.dataset.buyItem;if(e&&(t.preventDefault(),t.stopPropagation()),!a||!(a in r)||e?.disabled)return;const n=Date.now();this.lastBuyIntent?.itemDefinitionId===a&&n-this.lastBuyIntent.sentAt<350||(this.lastBuyIntent={itemDefinitionId:a,sentAt:n},this.shopFeedback=`Buying ${r[a].displayName}...`,this.render(),this.game.events.emit("wuland:buyMerchantItem",a))}handleHotbarPointerMove=t=>{if(!this.hotbarDrag)return;const e=Math.hypot(t.clientX-this.hotbarDrag.startX,t.clientY-this.hotbarDrag.startY);this.hotbarDrag.moved=this.hotbarDrag.moved||e>8};handleHotbarPointerUp=t=>{const e=this.hotbarDrag;if(!e)return;this.hotbarDrag=void 0;const a=document.elementFromPoint(t.clientX,t.clientY)?.closest("[data-hotbar-slot]");if(!e.moved){this.game.events.emit("wuland:selectHotbarSlot",e.slotIndex);return}if(!a){this.game.events.emit("wuland:discardHotbarItem",e.slotIndex);return}const n=Number.parseInt(a.dataset.hotbarSlot??"",10);Number.isInteger(n)&&this.game.events.emit("wuland:moveHotbarItem",{fromSlotIndex:e.slotIndex,toSlotIndex:n})};handleHotbarPointerDown(t){const e=t.target?.closest("[data-hotbar-slot]");if(!e)return;t.preventDefault();const a=Number.parseInt(e.dataset.hotbarSlot??"",10);Number.isInteger(a)&&(this.hotbarDrag={slotIndex:a,startX:t.clientX,startY:t.clientY,moved:!1})}cleanup(){this.game.events.off("wuland:progressUpdated",this.handleProgressUpdated,this),this.game.events.off("wuland:connectionUpdated",this.handleConnectionUpdated,this),this.game.events.off("wuland:toggleHelp",this.toggleHelp,this),this.game.events.off("wuland:toggleDebug",this.toggleDebug,this),this.game.events.off("wuland:openMerchantShop",this.openMerchantShop,this),this.game.events.off("wuland:chatHistory",this.handleChatHistory,this),this.game.events.off("wuland:chatMessage",this.handleChatMessage,this),this.game.events.off("wuland:shopFeedback",this.handleShopFeedback,this),this.game.events.off("wuland:focusChat",this.focusChatInput,this),window.removeEventListener("pointermove",this.handleHotbarPointerMove),window.removeEventListener("pointerup",this.handleHotbarPointerUp),window.removeEventListener("keydown",this.handleWindowKeydown,!0),this.root?.remove(),this.root=void 0}}const m=(s,t)=>{const e=new Map;return[...s,...t].forEach(a=>{e.set(a.messageId,a)}),[...e.values()].sort((a,n)=>Date.parse(a.sentAt)-Date.parse(n.sentAt)).slice(-100)},I=(s,t)=>{const e=r[t];return e.stackable&&s.find(n=>n.itemDefinitionId===t&&n.quantity>0&&n.quantity<e.maxStack)?!0:s.some(a=>!a.itemDefinitionId||a.quantity<=0)},g=s=>Math.max(0,Math.floor(s)).toLocaleString("en-US"),o=()=>window.matchMedia("(pointer: coarse)").matches||window.innerWidth<=860,k=()=>o()&&window.matchMedia("(orientation: portrait)").matches,C=s=>s.replace(/^Press F to /,"Interact: "),y=s=>s?s.iconAsset?`
    <img
      ...
      draggable="false"
    />
  `:`<strong class="item-icon-fallback">${c(s.iconText)}</strong>`:'<strong class="item-icon-fallback"></strong>',P=s=>{const t=r[s];return t.itemType==="weapon"?"Press Space to attack.":w(s)?"Press E to eat. Press G near another player to gift.":t.itemType==="consumable"?"Press E to use.":""},b=s=>s.replace(/&/g,"&amp;").replace(/"/g,"&quot;").replace(/</g,"&lt;").replace(/>/g,"&gt;"),c=s=>s.replace(/&/g,"&amp;").replace(/</g,"&lt;").replace(/>/g,"&gt;");export{T as UIScene};
